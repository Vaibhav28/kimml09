...
Gaussian Naive Bayes (GNB).\cite{bk:2004aplay}
...
From Table~\ref{tab:results} we can distinguish the fact that when PCA is used for feature extraction, the performance of Naive Bayes is lower than when other feature selection methods are used. By using an exact binomial test we found out that the confidence interval of the predictive performance of Naive Bayes when PCA is used is $95\% CI: 56.9 - 65.8$.

We can also distinguish the fact that when different structure is used for the data, the performance increases, 
