...
\author{Mattijs Meiboom, Robin Mills
, Tijn Schouten, Spyros Ioakeimidis}
\authorfooter{
\item
...
%% Abstract section.
\abstract{Functional magnetic resonance imaging (fMRI) is a brain mapping technique that allows researchers to measure activity in a subject's brain. This technique is typically used to determine which areas of the brain are involved in performing a certain task. Using this knowledge, we may also be able to determine what task was performed by looking at fMRI data. In this paper, we will discuss the use of machine learning algorithms for the classification of fMRI data. By using classifiers we will try to predict the cognitive state of subjects, based on their brain activity during that time.
} % end of abstract
...
\indent In this study we use the same StarPlus data as in the previously mentioned experiment, and use machine learning algorithms to decode the cognitive states of subjects when they are looking at pictures or when they are looking at sentences. An interesting difference between many applications of machine learning, and this study is that most machine learning algorithms have the goal of achieving the maximum possible discrimination between conditions or classes. However, for fMRI studies it is important that the pattern of activity that is identified by the algorithm is a pattern associated with the cognitive state. When using non-linear classifiers, it is possible to decode features predictive of the cognitive state, where the brain does not actually respond to these features. For example, when decoding the primary visual cortex with high enough spatial resolution, it would be possible to combine simple features with a non-linear neural network to decode the presence or absence of, for example, faces. However, this information is not actually processed in that part of the brain \cite{to:2012rev}. To avoid this problem it is recommended to only use linear classifiers when decoding cognitive states from fMRI data. The classifiers that we compare in this paper are Gaussian Naive Bayesian (GNB) classifiers, Linear Support Vector Machine (LSVM), and a Multivariate Gaussian (MVG) classifier.\\
       Insert alinea to explain shortly what our goal and used classifiers are (explained further in application?)\\

%Traditionally, it has been a challenge to relate between a simple cognitive state (such as `looking at a triangle`) and the corresponding activity of the brain \cite{Pone0001394}. This correspondence may even vary from subject to subject, making the challenge even harder. Since the introduction of fMRI, researchers have been able to acquire detailed measurements of brain activity, allowing for machine learning methods to be applied for the detection of patterns in fMRI data.
...
As a comparison measure, we used the performance of the classifiers on predicting the cognitive state that the subject was in. Table~\ref{tab:results} illustrates the final results. Rows indicate various feature selection methods. The letter \emph{M} means Columns depict the used classifiers. For the classifiers depicted with (*), we used different structure for the data that we fit to them (take example from section where the selection is introduced). 



explain the symbols on the table, explain what rows (feature selection methods) are and what columns are (different classifiers and some of them with different structure on the data)

first column naive bayes with pca 148 components, others common feature selection methods as described in Section~\ref{sec:featureSelection}

* means different structure of the data



classifiers performance do not differ significantly

because even PCA has accuracy 61.5\% we can be sure that all the classifiers differ significantly from 50\%
...
& \textbf{Naive Bayes} & \textbf{Linear SVC} & \textbf{Naive Bayes*} & \textbf{Multivariate*} \\ \hline
PCA & 61.5 \% &  - & - & - \\
M-ROIs & 70.2 \% & 72.5 \% & 74.8 \% & 76.7 \% \\
Norm-M-ROIs & 71.7 \% & 71.3 \% & 75.8 \% & 77.1 \% \\
Norm-M-SD-ROIs & 65.6 \% & 69.8 \% & 69.4 \% & 65.0 \%\\
Norm/Vox-M-ROIs & 73.1 \% & 76.7 \% & 76.9 \% & 79.6 \% \\
\end{tabular}}
\caption{Performance results of three classifiers using different methods for feature selection and structure.}
\label{tab:results}
\end{table*}
