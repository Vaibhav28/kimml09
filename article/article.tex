...
Temporal dimensionality reduction. As described in the introduction, the peak of the BOLD response is approximately 5 seconds after a stimulus is presented. This means that the first scans after a stimulus may not represent activity related to this stimulus and should not be used for classification. To examine which scans provide maximal class information, we plotted the average BOLD activity per scan for all trials of all subjects, given a trial type and ROI. As can be seen in Figure, scans 10 to 20 provide maximal information for the first stimulus, while scans 23 to 33 provide maximal information for the second stimulus. The other scans were excluded. As a comparison,      
...
Gaussian Naive Bayes (GNB).\cite{bk:2004aply}
